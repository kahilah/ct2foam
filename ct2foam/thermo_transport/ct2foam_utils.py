...
from thermo_transport import foam_writer as writer
...
def fit_ct_transport(data, poly_order=3):
    """
    data is a class object including T, mu, kappa and
    chemkin type order 3 fit
    """
    N = data.mu.shape[0]
    As = np.zeros(N)
    Ts = np.zeros(N)
...
def transport_fit_quality(data, transport_fits, output_dir, plot=True, rel_tol_sutherland=2e-2, rel_tol_Euken=2e-2, rel_tol_poly=5e-3):
    """
    data is class object including species names, mu, kappa, cv_mole and W.
    transport_fits is a tuple generated by the fit_ct_transport() function
    """
